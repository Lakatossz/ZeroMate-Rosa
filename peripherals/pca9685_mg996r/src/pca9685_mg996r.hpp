...
///
/// \brief This file defines a combination of PCA985 PWM regulator and MG996R servo motor that 
...
// ---------------------------------------------------------------------------------------------------------------------
class ZM_EXTERNAL_PERIPHERAL_API CPca9685_Mg996r final : public zero_mate::IExternal_Peripheral
{
public:
	...
	// -----------------------------------------------------------------------------------------------------------------
	/// \enum NCMD
	/// \brief Enumeration of different commands of the Step motor.
	// -----------------------------------------------------------------------------------------------------------------
	enum class NCMD : std::uint8_t
	{
		...
		Push_Forward = 0x81,				///< Moves motor clockwise
		Push_Backwards = 0xA4,				///< Moves motor counter-clockwise
		Reset_Position = 0xA5,				///< Resets position
		Read_Status = 0xA6,					///< Reads status
	};
	...
	/// \param sda_pin_idx I2C data pin index
	/// \param scl_pin_idx I2C clock pin index
	...
	// -----------------------------------------------------------------------------------------------------------------
	explicit CPca9685_Mg996r(const std::string& name,
	...
	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Renders the motor (GUI).
	// -----------------------------------------------------------------------------------------------------------------
	void Render() override;

	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Sets an ImGuiContext, so the motor can render itself as a GUI window.
	/// \param context ImGuiContext the motor uses to render itself
	// -----------------------------------------------------------------------------------------------------------------
...
private:
	...
	const char* const Pca9685_Mg996r_Config_File = "peripherals/pca9685_mg996r_config.json";
	...
	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Renders information about the motor
	// -----------------------------------------------------------------------------------------------------------------
	...
	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Initializes Motor and PWM regulator.
	// -----------------------------------------------------------------------------------------------------------------
	inline bool Init_Motor_State();
	...
	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Renders the number of the GPIO pin the motor is connected to.
	// -----------------------------------------------------------------------------------------------------------------
	inline void Render_Pin_Idx() const;

	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Renders the motor itself.
	// -----------------------------------------------------------------------------------------------------------------
	inline void Render_Motor();
	...
	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Pushes motor forward.
	/// \param steps Number of steps the motor will make
	// -----------------------------------------------------------------------------------------------------------------
	inline void Push_Forward(std::uint8_t steps);

	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Pushes motor backward.
	/// \param steps Number of steps the motor will make
	// -----------------------------------------------------------------------------------------------------------------
	inline void Push_Backward(std::uint8_t steps);

	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Resets motors position.
	// -----------------------------------------------------------------------------------------------------------------
	inline void Reset_Position();

	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Reads current status and puts it into the fifo, ready for reading.
	// -----------------------------------------------------------------------------------------------------------------
	...
	uint16_t m_steps_to_move = 0;
	bool m_first_part_loeaded = false;

	zero_mate::utils::CLogging_System* m_logging_system;        ///< Logging system

	ImGuiContext* m_context;									///< ImGUI context (rendering the GUI)
	bool m_output;												///< Current output from the motor

	int m_patient_idx;

	uint16_t m_current_angle;									///<

	uint32_t m_error_probability_referenced;					///< Probability of bubble found
