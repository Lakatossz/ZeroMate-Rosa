...
///
/// \brief This file implements a DS3231 RTC that can be connected to I2C pins at runtime as a shared lib.
///
/// You can find more information about the RTC itself over at https://www.laskakit.cz/user/related_files/ds3231.pdf
// ---------------------------------------------------------------------------------------------------------------------
...
CDS3231::CDS3231(const std::string& name,
...
nlohmann::json CDS3231::Parse_JSON_File(const std::string& path)
{
...
void CDS3231::Initialize()
{
		...
		!j["configuration"].empty())
	{
		...
		{
			this->m_error_probability_referenced = j["configuration"][0]["probability"][0].get<uint32_t>();
			this->m_error_probability = this->m_error_probability_referenced;
			if (this->m_error_probability) {
				m_incorrect_time = true;
			}
		}
	}
}
...
void CDS3231::Init_GPIO_Subscription()
{
	m_gpio_subscription.insert(m_sda_pin_idx);
	m_gpio_subscription.insert(m_scl_pin_idx);
}

void CDS3231::GPIO_Subscription_Callback([[maybe_unused]] std::uint32_t pin_idx)
{
...
void CDS3231::SCL_Pin_Change_Callback(bool curr_pin_state)
{
	m_scl_rising_edge = false;
...
void CDS3231::SDA_Pin_Change_Callback(bool curr_pin_state)
{
	m_sda_rising_edge = false;
...
void CDS3231::Updated_Type_Of_Processing_Data(std::uint8_t data)
{
...
void CDS3231::Log_Received_Data()
{
	std::stringstream ss{};
...
void CDS3231::Received_Transaction_Callback()
{
	Log_Received_Data();
...
void CDS3231::Process_CMD(std::uint8_t data)
{
...
void CDS3231::I2C_Receive_Start_Bit()
{
...
void CDS3231::I2C_Receive_Address()
{
...
void CDS3231::I2C_Receive_RW_Bit()
{
	// Read the RW bit.
...
void CDS3231::I2C_Receive_ACK_2()
{
...
void CDS3231::Set_ImGui_Context(void* context)
{
...
void CDS3231::Render_Information() const
{
	ImGui::Text("DS3231");
	ImGui::Text("I2C addr = 0x%X (%d dec)", m_address, m_address);
	ImGui::Text("Date time: %d/%d/%d - %d:%d:%d", m_day, (m_month + 1), (m_year + 1900), m_hour, m_minute, m_second);
}


void CDS3231::Render_Pins() const
{
	ImGui::Text("SDA pin: %d", m_sda_pin_idx);
	ImGui::Text("SCL pin: %d", m_scl_pin_idx);
}

void CDS3231::Render_Rtc()
{
...
void CDS3231::Get_Current_Time()
{
	auto now = std::chrono::system_clock::now();

	// Pøevedení na time_t (èitelné datum/èas)
	std::time_t now_time = std::chrono::system_clock::to_time_t(now);

	// Pøevedení na strukturovaný èas
	...
	m_output_fifo.push_back(m_minute);
	m_output_fifo.push_back(m_hour);
	m_output_fifo.push_back(m_day);
	m_output_fifo.push_back(m_date);
	m_output_fifo.push_back(m_month);
	m_output_fifo.push_back(m_year);
}

void CDS3231::Incorrect_Time()
{
	m_incorrect_time = true;
	m_error_probability = 100;
	m_logging_system->Error("Doslo k chybe casu");
}

void CDS3231::Correct_Time()
{
	m_incorrect_time = false;
	m_error_probability = m_error_probability_referenced;
	m_logging_system->Error("Chyba casu byla opravena");
}
...
extern "C"
{
			...
			zero_mate::utils::CLogging_System* logging_system)
	{
		...
		// Create an instance of an CDS3231.
		// clang-format off
		*peripheral = new (std::nothrow) CDS3231(name,
