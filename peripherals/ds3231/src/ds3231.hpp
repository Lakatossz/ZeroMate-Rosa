...
///
/// \brief This file defines a DS3231 RTC that can be connected to I2C pins at runtime as a shared library.
///
...
// ---------------------------------------------------------------------------------------------------------------------
/// \class CDS3231
/// \brief This class represents an DS3231 RTC.
// ---------------------------------------------------------------------------------------------------------------------
class ZM_EXTERNAL_PERIPHERAL_API CDS3231 final : public zero_mate::IExternal_Peripheral
{
public:
	...
	// -----------------------------------------------------------------------------------------------------------------
	explicit CDS3231(const std::string& name,
...
private:
	...
	const char* const CDS3231_Config_File = "peripherals/ds3231_config.json";
	...
	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Returns current date time value.
	// -----------------------------------------------------------------------------------------------------------------
	...
	/// \brief .
	// -----------------------------------------------------------------------------------------------------------------
	inline void Incorrect_Time();

	// -----------------------------------------------------------------------------------------------------------------
	/// \brief .
	// -----------------------------------------------------------------------------------------------------------------
	inline void Correct_Time();
	...
	uint8_t m_second;											///< Current second
	uint8_t m_minute;											///< Current minute
	uint8_t m_hour;												///< Current hour
	uint8_t m_day;												///< Current day
	uint8_t m_date;												///< Current date
	uint8_t m_month;											///< Current month
	uint8_t m_year;												///< Current year since 1900

	ImGuiContext* m_context;									///< ImGUI context (rendering the GUI)

	std::uint32_t m_error_probability_referenced;						///< Referenced probability of time failure
	std::uint32_t m_error_probability;								///< Probability of time failure

	bool m_incorrect_time;										///< Variable incidating false time measuring
};
