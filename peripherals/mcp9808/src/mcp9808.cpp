...
///
/// \brief This file implements a MCP9808 Temperature Sensor that can be connected to I2C pin at runtime as a shared library.
///
...
CMcp9808::CMcp9808(std::string name,
...
nlohmann::json CMcp9808::Parse_JSON_File(const std::string& path)
{
...
void CMcp9808::Initialize()
{
	nlohmann::json j = Parse_JSON_File(MCP9808_Config_File);
		...
		!j["configuration"].empty())
	{
			...
			j["configuration"][0]["referenced_value"][0].is_number_unsigned())
		{
			...
			this->m_current_temperature = this->m_referenced_value;
		}
...
void CMcp9808::Init_GPIO_Subscription()
{
	m_gpio_subscription.insert(m_sda_pin_idx);
	m_gpio_subscription.insert(m_scl_pin_idx);
}

void CMcp9808::GPIO_Subscription_Callback(std::uint32_t pin_idx)
{
...
void CMcp9808::Render_Information() const
{
	ImGui::Text("MCP9808 temperature sensor");
	ImGui::Text("I2C addr = 0x%X (%d dec)", m_address, m_address);
	ImGui::Text("Current temperature: %f", m_current_temperature);
}

void CMcp9808::Render_Pin_Idx() const
{
	ImGui::Text("SDA pin: %d", m_sda_pin_idx);
	ImGui::Text("SCL pin: %d", m_scl_pin_idx);
}

void CMcp9808::Render_Mcp9808()
{
...
void CMcp9808::I2C_Receive_Start_Bit()
{
...
void CMcp9808::I2C_Receive_Address()
{
...
void CMcp9808::I2C_Receive_RW_Bit()
{
	// Read the RW bit.
...
void CMcp9808::I2C_Receive_Data()
{
...
void CMcp9808::SCL_Pin_Change_Callback(bool curr_pin_state)
{
	m_scl_rising_edge = false;
...
void CMcp9808::SDA_Pin_Change_Callback(bool curr_pin_state)
{
	m_sda_rising_edge = false;
...
void CMcp9808::Updated_Type_Of_Processing_Data(std::uint8_t data)
{
...
void CMcp9808::Log_Received_Data()
{
	std::stringstream ss{};
...
void CMcp9808::Received_Transaction_Callback()
{
	Log_Received_Data();
...
void CMcp9808::Process_CMD(std::uint8_t data)
{
...
void CMcp9808::Init_Transaction()
{
	m_fifo.clear();
...
void CMcp9808::I2C_Receive_ACK_2()
{
...
void CMcp9808::Read_Temperature()
{
	std::cout << "Ctu teplotu\n";

	if (m_higher_temperature) {
		m_current_temperature = m_higher_value;
	}
	else if (m_lower_temperature) {
		m_current_temperature = m_lower_value;
	}
	else {
		m_current_temperature = m_referenced_value;
	}

	std::stringstream ss{};

	ss << static_cast<double>(m_current_temperature);

	m_logging_system->Info(("Temperature: " + ss.str()).c_str());
	...
	for (int i = 0; i < sizeof(float); i++) {
		std::cout << "bytes[i]: " << static_cast<std::uint32_t>(bytes[i]) << std::endl;
...
extern "C"
{
			...
			[[maybe_unused]] zero_mate::utils::CLogging_System* logging_system)
	{
		...
		*peripheral = new (std::nothrow) CMcp9808(name,
