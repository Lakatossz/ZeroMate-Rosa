#include "ads1115_pbmh.hpp"

CAds1115_Pbmh::CAds1115_Pbmh(std::string name,
	...
	, m_referenced_value{ 0 }
	, m_current_pressure{ 0 }
	, m_higher_value{ 0 }
	, m_lower_value{ 0 }
	, m_higher_pressure{ false }
	, m_lower_pressure{ false }
...
CAds1115_Pbmh::~CAds1115_Pbmh() = default;

nlohmann::json CAds1115_Pbmh::Parse_JSON_File(const std::string& path)
{
...
void CAds1115_Pbmh::Initialize()
{
		...
		!j["configuration"].empty())
	{
			...
			j["configuration"][0]["start_with"][0].is_number_unsigned())
		{
			...
			if (start_with == 0) {
				this->m_higher_pressure = false;
				this->m_lower_pressure = false;
			}
			else if (start_with == 1) {
				this->m_higher_pressure = true;
				this->m_lower_pressure = !this->m_higher_pressure;
			}
			else if (start_with == 2) {
				this->m_lower_pressure = true;
				this->m_higher_pressure = !this->m_lower_pressure;
			}
		}
	}
}

void CAds1115_Pbmh::Init_GPIO_Subscription()
{
	m_gpio_subscription.insert(m_sda_pin_idx);
	m_gpio_subscription.insert(m_scl_pin_idx);
}

void CAds1115_Pbmh::GPIO_Subscription_Callback([[maybe_unused]] std::uint32_t pin_idx)
{
...
void CAds1115_Pbmh::Set_ImGui_Context(void* context)
{
	// Store the ImGUI Context.
	m_context = static_cast<ImGuiContext*>(context);
}

void CAds1115_Pbmh::Render()
{
	...
	if (ImGui::Begin(m_name.c_str()))
	{
		Render_Information();
		Render_Pins();
		Render_Pressure_Sensor();
	}

	ImGui::End();
}

void CAds1115_Pbmh::Render_Information() const
{
	ImGui::Text("ADS1115 ADC and Pbmh pressure sensor");
	ImGui::Text("I2C addr = 0x%X (%d dec)", m_address, m_address);
	ImGui::Text("Current pressure: %f", m_current_pressure);
}

void CAds1115_Pbmh::Render_Pins() const
{
	ImGui::Text("SDA pin: %d", m_sda_pin_idx);
	ImGui::Text("SCL pin: %d", m_scl_pin_idx);
}

void CAds1115_Pbmh::Render_Pressure_Sensor()
{
	if (!m_lower_pressure && ImGui::Checkbox("Higher pressure", &m_higher_pressure))
	{
		if (m_higher_pressure == true) {
			m_logging_system->Error("Byl detekovan vyssi tlak");
		}
		Read_Pressure();
	}
	else if (!m_higher_pressure && ImGui::Checkbox("Lower pressure", &m_lower_pressure))
	{
		if (m_lower_pressure == true) {
...
void CAds1115_Pbmh::I2C_Receive_Start_Bit()
{
...
void CAds1115_Pbmh::I2C_Receive_Address()
{
...
void CAds1115_Pbmh::I2C_Receive_RW_Bit()
{
	// Read the RW bit.
...
void CAds1115_Pbmh::I2C_Send_Data()
{
	if (m_transaction.data_idx > 0) {
		--m_transaction.data_idx;
		// Store the data into the FIFO only if it is meant to be for us.
...
void CAds1115_Pbmh::SCL_Pin_Change_Callback(bool curr_pin_state)
{
	m_scl_rising_edge = false;
...
void CAds1115_Pbmh::SDA_Pin_Change_Callback(bool curr_pin_state)
{
	m_sda_rising_edge = false;
...
void CAds1115_Pbmh::Updated_Type_Of_Processing_Data(std::uint8_t data)
{
...
void CAds1115_Pbmh::Log_Received_Data()
{
	std::stringstream ss{};
...
void CAds1115_Pbmh::Received_Transaction_Callback()
{
	Log_Received_Data();
...
void CAds1115_Pbmh::Process_Data(std::uint8_t data)
{
	if (m_fifo.size() > 2) {
...
void CAds1115_Pbmh::Process_CMD(std::uint8_t data)
{
...
void CAds1115_Pbmh::Init_Transaction()
{
	m_fifo.clear();
...
void CAds1115_Pbmh::I2C_Receive_ACK_2()
{
...
void CAds1115_Pbmh::Read_Pressure()
{
	if (m_higher_pressure) {
		m_current_pressure = this->m_higher_value;
	}
	else if (m_lower_pressure) {
		m_current_pressure = this->m_lower_value;
	}
	else {
		m_current_pressure = this->m_referenced_value;
	}
...
extern "C"
{
			...
			[[maybe_unused]] zero_mate::utils::CLogging_System* logging_system)
	{
		...
		*peripheral = new (std::nothrow) CAds1115_Pbmh(name,
