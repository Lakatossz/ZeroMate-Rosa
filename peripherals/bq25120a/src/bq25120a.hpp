...
///
/// \brief This file defines a BQ25120a battery that can be connected to I2C pins at runtime as a shared lib.
///
...
// ---------------------------------------------------------------------------------------------------------------------
class ZM_EXTERNAL_PERIPHERAL_API CBq25120a final : public zero_mate::IExternal_Peripheral
{
public:
	...
	enum class NCMD : std::uint8_t
	{
		...
		Write_Init = 0xE4,					///< Read value
		Write_Safe = 0xE5,					///< Read value
	};
	...
	// -----------------------------------------------------------------------------------------------------------------
	explicit CBq25120a(const std::string& name,
...
private:
	...
	const char* const Bq25120a_Config_File = "peripherals/bq25120a_config.json";

	[[nodiscard]] inline nlohmann::json Parse_JSON_File(const std::string& path);

	inline void Set_Parameter(nlohmann::json j, std::string name, float& variable);

	inline void Set_Parameter_And_Referenced(nlohmann::json j, std::string name, float& referenced, float& variable);
	...
	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Renders the information of the RTC.
	// -----------------------------------------------------------------------------------------------------------------
	inline void Render_Information() const;

	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Renders the number of the GPIO pin the RTC is connected to.
	// -----------------------------------------------------------------------------------------------------------------
	inline void Render_Pins() const;

	// -----------------------------------------------------------------------------------------------------------------
	/// \brief Renders the RTC itself.
	// -----------------------------------------------------------------------------------------------------------------
	inline void Render_Battery();
	...
	float m_capacity_downfall;
	float m_referenced_temperature;
	float m_current_temperature;
	float m_higher_temperature_value;
	float m_lower_temperature_value;
	bool m_higher_temperature;
	bool m_lower_temperature;
	float m_referenced_charge;
	float m_current_charge;
	float m_higher_charge_value;
	float m_lower_charge_value;
	bool m_higher_charge;
	bool m_lower_charge;
	float m_referenced_voltage;
	float m_current_voltage;
	float m_higher_voltage_value;
	float m_lower_voltage_value;
	bool m_higher_voltage;
	bool m_lower_voltage;
	float m_referenced_current;
	float m_current_current;
	float m_higher_current_value;
	float m_lower_current_value;
	bool m_higher_current;
	bool m_lower_current;

	bool m_error_occured;

	std::uint32_t charge_cycle_time = 60;
